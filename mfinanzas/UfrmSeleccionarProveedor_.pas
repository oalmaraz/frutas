unit UfrmSeleccionarProveedor_;
...
uses
  ...
  Db, IBCustomDataSet,
  IBDatabase,  StdCtrls, Mask, DBCtrls,  
       ExtCtrls,
   Menus, Buttons, ComCtrls, Grids, DBGrids, IBQuery;

type
  TfrmSeleccionarProveedor_ = class(TForm)
    pmCampos: TPopupMenu;
    SCID1: TMenuItem;
    itemCLAVE: TMenuItem;
...
var
  frmSeleccionarProveedor_: TfrmSeleccionarProveedor_;

implementation

{$R *.DFM}

procedure TfrmSeleccionarProveedor_.setDatabase(valor : TIBDatabase);
begin
   qyBusqueda.Database := valor;
   sqlSelec.DataBase   := valor;
end;

procedure TfrmSeleccionarProveedor_.setTransaction(valor : TIBTransaction);
begin
...
function TfrmSeleccionarProveedor_.abrirConsulta : boolean;
var
   tmp : boolean;
begin
   tmp := true;
...
function TfrmSeleccionarProveedor_.getMONEDA : integer;
begin
   Result := dsBusqueda.DataSet.FieldByName('MONEDA').AsInteger;
end;

function TfrmSeleccionarProveedor_.getCCOMERCIAL : integer;
begin
   Result := dsBusqueda.DataSet.FieldByName('FORMAS_DE_PAGO').AsInteger;
end;

function TfrmSeleccionarProveedor_.getDESC_PORC : double;
begin
   Result := dsBusqueda.DataSet.FieldByName('DESCUENTO_PORC').AsFloat;
end;

function TfrmSeleccionarProveedor_.getTELEFONO : string;
begin
   Result := dsBusqueda.DataSet.FieldByName('TEL_PRINCIPAL').AsString;
end;

function TfrmSeleccionarProveedor_.getRSOCIAL : string;
begin
   result := dsBusqueda.DataSet.FieldByName('RAZON_SOCIAL').AsString;
end;

function TfrmSeleccionarProveedor_.getFacturarA : string;
begin
...
procedure TfrmSeleccionarProveedor_.setCLAVE(valor : string);
begin
   edDato.Text := valor;
end;

procedure TfrmSeleccionarProveedor_.setAceptar(valor : boolean);
begin
   BAceptar := valor;
   if BAceptar then
      Close;
...
function TfrmSeleccionarProveedor_.getID : integer;
begin
   Result := dsBusqueda.DataSet.FieldByName('ID').AsInteger;
end;

function TfrmSeleccionarProveedor_.getCLAVE : string;
begin
   Result := dsBusqueda.DataSet.FieldByName('CLAVE').AsString;
end;

function TfrmSeleccionarProveedor_.getNOMBRE : string;
begin
   Result :=  dsBusqueda.DataSet.FieldByName('NOMBRE').AsString;
end;

function TfrmSeleccionarProveedor_.getAceptar : boolean;
begin
   Result := BAceptar;
end;

procedure TfrmSeleccionarProveedor_.FormCreate(Sender: TObject);
begin
   ACEPTAR := false;
...
procedure TfrmSeleccionarProveedor_.buInicioClick(Sender: TObject);
begin
   dgExplorar.DataSource.DataSet.First;
end;

procedure TfrmSeleccionarProveedor_.buAnteriorClick(Sender: TObject);
begin
   dgExplorar.DataSource.DataSet.Prior;
end;

procedure TfrmSeleccionarProveedor_.buSiguienteClick(Sender: TObject);
begin
   dgExplorar.DataSource.DataSet.Next;
end;

procedure TfrmSeleccionarProveedor_.buFinClick(Sender: TObject);
begin
   dgExplorar.DataSource.DataSet.Last;
end;

procedure TfrmSeleccionarProveedor_.ibSalirClick(Sender: TObject);
begin
   Close;
end;

procedure TfrmSeleccionarProveedor_.dgExplorarKeyUp(Sender: TObject;
...
procedure TfrmSeleccionarProveedor_.buTodosClick(Sender: TObject);
begin
...
procedure TfrmSeleccionarProveedor_.buBusquedaClick(Sender: TObject);
begin
   buEjecutar.Click;
end;

procedure TfrmSeleccionarProveedor_.edDatoKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmSeleccionarProveedor_.buEjecutarClick(Sender: TObject);
var
  vLogica    : string;
  vCampo     : string;
  vSentencia : string;
...
procedure TfrmSeleccionarProveedor_.dgExplorarDblClick(Sender: TObject);
begin
   if not(qyBusqueda.IsEmpty) then
      ACEPTAR := true
end;

procedure TfrmSeleccionarProveedor_.SCID1Click(Sender: TObject);
begin
   buBusqueda.Caption := 'SP.ID';
end;

procedure TfrmSeleccionarProveedor_.itemCLAVEClick(Sender: TObject);
begin
   buBusqueda.Caption := 'S.CLAVE';
end;

procedure TfrmSeleccionarProveedor_.itemNOMBREClick(Sender: TObject);
begin
   buBusqueda.Caption := 'S.NOMBRE';
end;

procedure TfrmSeleccionarProveedor_.itemRAZONClick(Sender: TObject);
begin
   buBusqueda.Caption := 'S.RAZON_SOCIAL';
end;

procedure TfrmSeleccionarProveedor_.itemCompraMinimaClick(Sender: TObject);
begin
   buBusqueda.Caption := 'SP.COMPRA_MINIMA';
end;

procedure TfrmSeleccionarProveedor_.itemTELEFONOClick(Sender: TObject);
begin
   buBusqueda.Caption := 'S.TEL_PRINCIPAL';
end;

procedure TfrmSeleccionarProveedor_.FormDestroy(Sender: TObject);
begin
   sqlSelec.Free;
end;
