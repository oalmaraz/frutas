inherited frmCapturar_compradores: TfrmCapturar_compradores
  Left = 390
  Top = 98
  Caption = 'Compradores'
  ExplicitWidth = 320
  ExplicitHeight = 240
  PixelsPerInch = 96
  TextHeight = 13
  inherited paLienzo: TPanel
    inherited fpMasDatos: TPanel
      Top = 450
      Height = 73
      ExplicitTop = 450
      ExplicitHeight = 73
      inherited fpMasDatoDentro: TPanel
        Height = 71
        ExplicitHeight = 71
        object Label12: TLabel
          Left = 5
          Top = 24
          Width = 59
          Height = 13
          Caption = 'Clasificaci'#243'&n'
          FocusControl = leClasificacion
          Transparent = True
        end
        object Label13: TLabel
          Left = 224
          Top = 24
          Width = 48
          Height = 13
          Caption = 'Tabulador'
          Transparent = True
        end
        object Label14: TLabel
          Left = 432
          Top = 24
          Width = 67
          Height = 13
          Caption = 'Departa&mento'
          FocusControl = leDepartamentos
          Transparent = True
        end
        object sbRefreshClasificacion: TSpeedButton
          Left = 200
          Top = 40
          Width = 21
          Height = 21
          Hint = 'Actualizar'
          Glyph.Data = {
            
            04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
          ParentShowHint = False
          ShowHint = True
          OnClick = sbRefreshClasificacionClick
        end
        object sbRefreshTabulador: TSpeedButton
          Left = 408
          Top = 40
          Width = 21
          Height = 21
          Hint = 'Actualizar'
          Glyph.Data = {
            
            04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
          ParentShowHint = False
          ShowHint = True
          OnClick = sbRefreshTabuladorClick
        end
        object sbRefreshDepartamento: TSpeedButton
          Left = 608
          Top = 40
          Width = 21
          Height = 21
          Hint = 'Actualizar'
          Glyph.Data = {
            
            04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
          ParentShowHint = False
          ShowHint = True
          OnClick = sbRefreshDepartamentoClick
        end
        object leClasificacion: TDBLookupComboBox
          Left = 5
          Top = 40
          Width = 193
          Height = 21
          DataField = 'descripcion'
          DataSource = dsFuente
          TabOrder = 0
        end
        object leTabulador: TDBLookupComboBox
          Left = 224
          Top = 40
          Width = 182
          Height = 21
          DataField = 'clave'
          DataSource = dsFuente
          TabOrder = 1
        end
        object leDepartamentos: TDBLookupComboBox
          Left = 432
          Top = 40
          Width = 174
          Height = 21
          DataField = 'clave_departamento'
          DataSource = dsFuente
          TabOrder = 2
        end
      end
    end
    inherited pcSujetos: TPageControl
      Height = 448
      ExplicitHeight = 448
      inherited tsDatos: TTabSheet
        inherited fpDatos: TPanel
          Height = 400
          ExplicitHeight = 400
          inherited laCtaContable: TLabel
            Top = 245
            ExplicitTop = 245
          end
          inherited buActCtas: TBitBtn
            Left = 173
            Top = 260
            ExplicitLeft = 173
            ExplicitTop = 260
          end
          inherited buLimCtas: TBitBtn
            Left = 196
            Top = 260
            ExplicitLeft = 196
            ExplicitTop = 260
          end
        end
      end
      inherited tsDomicilio: TTabSheet
        inherited fpDomicilio: TPanel
          Height = 400
          ExplicitHeight = 400
        end
      end
      inherited tsContactos: TTabSheet
        inherited paContactos: TPanel
          Height = 234
          ExplicitHeight = 234
        end
        inherited fpContactosDetalle: TPanel
          Height = 234
          ExplicitHeight = 234
          inherited dgContactos: TDBGrid
            Height = 202
          end
        end
      end
      object tsZonas: TTabSheet
        Hint = 'Zonas'
        Caption = 'Zonas'
        ImageIndex = 3
        ParentShowHint = False
        ShowHint = True
        OnShow = tsZonasShow
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object dgZonas: TDBGrid
          Left = 0
          Top = 59
          Width = 903
          Height = 341
          Align = alClient
          Color = clInfoBk
          DataSource = dsZonas
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
        end
        object fpZonaV: TPanel
          Left = 0
          Top = 0
          Width = 903
          Height = 59
          Align = alTop
          BevelOuter = bvNone
          Caption = 'click zonas'
          TabOrder = 1
          object fpZonas: TPanel
            Left = 0
            Top = 0
            Width = 903
            Height = 59
            Align = alClient
            BevelOuter = bvNone
            Caption = 'fpZonas'
            Color = 14933717
            TabOrder = 0
            object ibNuevaZona: TBitBtn
              Left = 8
              Top = 21
              Width = 79
              Height = 34
              Hint = 'Nuevo'
              Caption = 'nueva'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              Glyph.Data = {
                
                FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
              Layout = blGlyphRight
              ParentFont = False
              ParentShowHint = False
              ShowHint = True
              TabOrder = 0
              OnClick = ibNuevaZonaClick
            end
            object ibEliminarZona: TBitBtn
              Left = 89
              Top = 21
              Width = 79
              Height = 34
              Hint = 'Eliminar'
              Caption = 'eliminar'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              Glyph.Data = {
                
                FFFF60CF6060CF60404740707770404740FFCFCF60303060CF60}
              Layout = blGlyphRight
              ParentFont = False
              ParentShowHint = False
              ShowHint = True
              TabOrder = 1
              OnClick = ibEliminarZonaClick
            end
          end
        end
      end
    end
  end
  inherited dsFuente: TDataSource
    Top = 240
  end
  inherited TCapturar: TIBTable
    FieldDefs = <
      item
        Name = 'ID'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'SUJETO'
        DataType = ftInteger
      end
      item
        Name = 'CLASIFICACION'
        DataType = ftInteger
      end
      item
        Name = 'TABULADOR'
        DataType = ftInteger
      end
      item
        Name = 'DEPARTAMENTO'
        DataType = ftInteger
      end>
    IndexDefs = <
      item
        Name = 'RDB$PRIMARY86'
        Fields = 'ID'
        Options = [ixPrimary, ixUnique]
      end
      item
        Name = 'RDB$FOREIGN87'
        Fields = 'SUJETO'
      end
      item
        Name = 'RDB$FOREIGN88'
        Fields = 'CLASIFICACION'
      end
      item
        Name = 'RDB$FOREIGN89'
        Fields = 'TABULADOR'
      end>
    IndexFieldNames = 'ID'
    MasterFields = 'R_ID'
    StoreDefs = True
    TableName = 'SUJETOS_COMPRADORES'
    Top = 240
    object TCapturarID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object TCapturarSUJETO: TIntegerField
      FieldName = 'SUJETO'
    end
    object TCapturarCLASIFICACION: TIntegerField
      FieldName = 'CLASIFICACION'
    end
    object TCapturarTABULADOR: TIntegerField
      FieldName = 'TABULADOR'
    end
    object TCapturarDEPARTAMENTO: TIntegerField
      FieldName = 'DEPARTAMENTO'
    end
    object TCapturardescripcion: TStringField
      FieldKind = fkLookup
      FieldName = 'descripcion'
      LookupDataSet = TClasificaciones
      LookupKeyFields = 'ID'
      LookupResultField = 'DESCRIPCION'
      KeyFields = 'CLASIFICACION'
      Size = 50
      Lookup = True
    end
    object TCapturarclave: TStringField
      FieldKind = fkLookup
      FieldName = 'clave'
      LookupDataSet = TTabuladores
      LookupKeyFields = 'ID'
      LookupResultField = 'CLAVE'
      KeyFields = 'TABULADOR'
      Lookup = True
    end
    object TCapturarclave_departamento: TStringField
      FieldKind = fkLookup
      FieldName = 'clave_departamento'
      LookupDataSet = TDepartamentos
      LookupKeyFields = 'ID'
      LookupResultField = 'CLAVE'
      KeyFields = 'DEPARTAMENTO'
      Lookup = True
    end
  end
  inherited spID: TIBStoredProc
    Top = 240
    ParamData = <
      item
        DataType = ftInteger
        Name = 'R_ID'
        ParamType = ptOutput
      end
      item
        DataType = ftString
        Name = 'P_QUIEN'
        ParamType = ptInput
        Value = 'COMPRADORES'
      end>
  end
  inherited TSujetos: TIBTable
    Top = 271
  end
  inherited dsSujetos: TDataSource
    Top = 271
  end
  inherited TTelefonos: TIBTable
    Top = 271
  end
  inherited dsTelefonos: TDataSource
    Top = 271
  end
  inherited TDomicilios: TIBTable
    Top = 301
  end
  inherited dsDomicilios: TDataSource
    Top = 301
  end
  inherited TContactos: TIBTable
    Top = 332
  end
  inherited dsContactos: TDataSource
    Top = 332
  end
  inherited TTelefonosContactos: TIBTable
    Top = 332
  end
  inherited dsTelefonosContactos: TDataSource
    Top = 332
  end
  inherited qyCiudades: TIBQuery
    Top = 301
  end
  inherited dsCiudades: TDataSource
    Top = 301
  end
  inherited spIDContactos: TIBStoredProc
    Left = 272
  end
  inherited ilImagenes: TImageList
    Left = 548
    Bitmap = {
      
      000000000000}
  end
  object TClasificaciones: TIBTable
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'ID'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'DESCRIPCION'
        DataType = ftString
        Size = 50
      end>
    IndexDefs = <
      item
        Name = 'RDB$PRIMARY9'
        Fields = 'ID'
        Options = [ixPrimary, ixUnique]
      end>
    IndexFieldNames = 'ID'
    StoreDefs = True
    TableName = 'CLASIFICACIONES'
    UniDirectional = False
    Left = 160
    Top = 93
    object TClasificacionesID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object TClasificacionesDESCRIPCION: TIBStringField
      FieldName = 'DESCRIPCION'
      Size = 50
    end
  end
  object TTabuladores: TIBTable
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'ID'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'CLAVE'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'LIMITE_INF'
        DataType = ftFloat
      end
      item
        Name = 'LIMITE_SUP'
        DataType = ftFloat
      end
      item
        Name = 'BASE_MONTO'
        DataType = ftFloat
      end
      item
        Name = 'PORCENTAJE'
        DataType = ftFloat
      end>
    IndexDefs = <
      item
        Name = 'RDB$PRIMARY10'
        Fields = 'ID'
        Options = [ixPrimary, ixUnique]
      end>
    IndexFieldNames = 'ID'
    StoreDefs = True
    TableName = 'TABULADORES'
    UniDirectional = False
    Left = 360
    Top = 77
    object TTabuladoresID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object TTabuladoresCLAVE: TIBStringField
      FieldName = 'CLAVE'
    end
    object TTabuladoresLIMITE_INF: TFloatField
      FieldName = 'LIMITE_INF'
    end
    object TTabuladoresLIMITE_SUP: TFloatField
      FieldName = 'LIMITE_SUP'
    end
    object TTabuladoresBASE_MONTO: TFloatField
      FieldName = 'BASE_MONTO'
    end
    object TTabuladoresPORCENTAJE: TFloatField
      FieldName = 'PORCENTAJE'
    end
  end
  object dsClasificaciones: TDataSource
    DataSet = TClasificaciones
    Left = 192
    Top = 93
  end
  object dsTabuladores: TDataSource
    DataSet = TTabuladores
    Left = 392
    Top = 77
  end
  object TZonas: TIBTable
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    OnCalcFields = TZonasCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'ID'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'COMPRADOR'
        DataType = ftInteger
      end
      item
        Name = 'ZONA'
        DataType = ftInteger
      end>
    IndexDefs = <
      item
        Name = 'RDB$PRIMARY90'
        Fields = 'ID'
        Options = [ixPrimary, ixUnique]
      end
      item
        Name = 'RDB$FOREIGN91'
        Fields = 'VENDEDOR'
      end
      item
        Name = 'RDB$FOREIGN92'
        Fields = 'ZONA'
      end>
    IndexFieldNames = 'COMPRADOR'
    MasterFields = 'ID'
    MasterSource = dsFuente
    StoreDefs = True
    TableName = 'COMPRADORES_ZONAS'
    UniDirectional = False
    Left = 304
    Top = 304
    object TZonasID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object TZonasCOMPRADOR: TIntegerField
      FieldName = 'COMPRADOR'
    end
    object TZonasZONA: TIntegerField
      FieldName = 'ZONA'
    end
    object TZonasdescripcion: TStringField
      FieldKind = fkCalculated
      FieldName = 'descripcion'
      Size = 202
      Calculated = True
    end
  end
  object dsZonas: TDataSource
    DataSet = TZonas
    OnDataChange = dsZonasDataChange
    Left = 304
    Top = 333
  end
  object qyBuscar: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    DataSource = dsZonas
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM BUSCAR_ESTADOS_O_CIUDADES(:ZONA)')
    Left = 304
    Top = 364
    ParamData = <
      item
        DataType = ftInteger
        Name = 'ZONA'
        ParamType = ptUnknown
      end>
    object qyBuscarR_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCRIPCION'
      ReadOnly = True
      Size = 202
    end
  end
  object spDelZona: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'DEL_ZONA_COMPRADOR'
    Left = 272
    Top = 333
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_ZONA'
        ParamType = ptInput
        Value = '0'
      end>
  end
  object TDepartamentos: TIBTable
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'ID'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'CLAVE'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'DESCRIPCION'
        DataType = ftString
        Size = 100
      end>
    IndexDefs = <
      item
        Name = 'RDB$PRIMARY21'
        Fields = 'ID'
        Options = [ixPrimary, ixUnique]
      end>
    StoreDefs = True
    TableName = 'DEPARTAMENTOS'
    UniDirectional = False
    Left = 584
    Top = 77
    object TDepartamentosID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object TDepartamentosCLAVE: TIBStringField
      FieldName = 'CLAVE'
    end
    object TDepartamentosDESCRIPCION: TIBStringField
      FieldName = 'DESCRIPCION'
      Size = 100
    end
  end
  object dsDepartamentos: TDataSource
    DataSet = TDepartamentos
    Left = 616
    Top = 77
  end
end
