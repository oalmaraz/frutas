object frmSeleccionarExtension: TfrmSeleccionarExtension
  Left = 462
  Top = 100
  Caption = 'Seleccionar Extension'
  ClientHeight = 0
  ClientWidth = 124
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  WindowState = wsMaximized
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object paBusqueda: TPanel
    Left = 0
    Top = 0
    Width = 124
    Height = 52
    Align = alTop
    BevelOuter = bvNone
    Caption = 'Seleccion'
    Color = 12505816
    TabOrder = 0
    ExplicitWidth = 790
    object buBusqueda: TBitBtn
      Left = 4
      Top = 22
      Width = 188
      Height = 27
      Caption = 'CE.NOMBRE'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        FF7F1F7C1F7C}
      ParentFont = False
      Spacing = 9
      TabOrder = 3
      OnClick = buBusquedaClick
    end
    object buEjecutar: TBitBtn
      Left = 339
      Top = 20
      Width = 68
      Height = 31
      Caption = '&Ejecutar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
      ParentFont = False
      TabOrder = 1
      OnClick = buEjecutarClick
    end
    object buTodos: TBitBtn
      Left = 409
      Top = 20
      Width = 68
      Height = 31
      Caption = '&Todos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = buTodosClick
    end
    object edDato: TEdit
      Left = 195
      Top = 25
      Width = 141
      Height = 21
      CharCase = ecUpperCase
      TabOrder = 0
      OnKeyPress = edDatoKeyPress
    end
  end
  object dgExplorar: TDBGrid
    Left = 0
    Top = 52
    Width = 124
    Height = 372
    Align = alClient
    Color = clInfoBk
    DataSource = dsBusqueda
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnDblClick = dgExplorarDblClick
    OnKeyUp = dgExplorarKeyUp
  end
  object Panel2: TPanel
    Left = 0
    Top = -22
    Width = 124
    Height = 22
    Align = alBottom
    TabOrder = 2
    ExplicitTop = 424
    ExplicitWidth = 790
    object buInicio: TBitBtn
      Left = 94
      Top = 2
      Width = 26
      Height = 18
      Glyph.Data = {
        }
      TabOrder = 0
    end
    object buAnterior: TBitBtn
      Left = 123
      Top = 2
      Width = 26
      Height = 18
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      NumGlyphs = 2
      TabOrder = 1
    end
    object buFin: TBitBtn
      Left = 180
      Top = 2
      Width = 26
      Height = 18
      Glyph.Data = {
        }
      TabOrder = 2
    end
    object buSiguiente: TBitBtn
      Left = 151
      Top = 2
      Width = 26
      Height = 18
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      NumGlyphs = 2
      TabOrder = 3
    end
    object ibSalir: TBitBtn
      Left = 212
      Top = 2
      Width = 36
      Height = 18
      Glyph.Data = {
        
        0000}
      TabOrder = 4
    end
    object deID: TDBEdit
      Left = 1
      Top = 2
      Width = 88
      Height = 18
      BorderStyle = bsNone
      Enabled = False
      TabOrder = 5
    end
  end
  object IBDatabase1: TIBDatabase
    DatabaseName = 'D:\proyectos\apm\Bases\SISTEMA.AWR'
    Params.Strings = (
      'user_name=SYSDBA'
      'password=masterkey')
    LoginPrompt = False
    DefaultTransaction = IBTransaction1
    ServerType = 'IBServer'
    SQLDialect = 1
    Left = 112
    Top = 128
  end
  object IBTransaction1: TIBTransaction
    Params.Strings = (
      'read_committed'
      'rec_version'
      'nowait')
    Left = 112
    Top = 160
  end
  object dsBusqueda: TDataSource
    DataSet = qyBusqueda
    Left = 208
    Top = 160
  end
  object qyBusqueda: TIBQuery
    Database = IBDatabase1
    Transaction = IBTransaction1
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT CE.ID CE_ID,'
      '       CE.CLAVE CE_CLAVE,'
      '       CE.NOMBRE CE_NOMBRE,'
      '       CE.TELEFONO CE_TELEFONO,'
      '       CE.LIMITE_CREDITO CE_LIMITE_CREDITO,'
      '       CE.PROVEEDOR CE_REF_CLI_PROV,'
      '       CE.DIRECCION CE_DIRECCION,'
      '       D.CALLE CE_CALLE,'
      '       D.NUM_EXT CE_NUM_EXT,'
      '       D.LETRA CE_LETRA,'
      '       D.NUM_INT CE_NUM_INT,'
      '       D.CP CE_CP,'
      '       D.COLONIA CE_COLONIA,'
      '       D.CIUDAD CE_CIUDAD, '
      '       C.DESCRIPCION CE__CIUDAD,'
      '       E.DESCRIPCION CE__ESTADO,'
      '       P.DESCRIPCION CE__PAIS, '
      '       CE.TRANSPORTISTA CE_TRANSPORTISTA,'
      '       S2.NOMBRE CE_T_NOMBRE,'
      '       CE.RFC CE_RFC,'
      '       CE.FLAG_FACTURAR CE_FLAG_FACT,'
      '       CE.REMITENTE CE_REMITENTE,'
      '       S3.NOMBRE CE_R_NOMBRE,'
      '       CE.ESTATUS CE_ESTATUS,'
      '       CE.SUJETO_CLIENTE C_CLIENTE,'
      '       S.CLAVE C_CLAVE,'
      '       S.NOMBRE C_NOMBRE, '
      '       SC.LIMITE_CREDITO C_LIMITE_CREDITO,'
      '       SC.DESCUENTO_PORC C_DESC_PORC, '
      '       S.DOMICILIO C_DOMICILIO, '
      '       D2.CALLE C_CALLE,'
      '       D2.NUM_EXT C_NUM_EXT,'
      '       D2.LETRA C_LETRA,'
      '       D2.NUM_INT C_NUM_INT,'
      '       D2.CP C_CP,'
      '       D2.COLONIA C_COLONIA,'
      '       D2.CIUDAD C_CIUDAD, '
      '       C2.DESCRIPCION C__CIUDAD,'
      '       E2.DESCRIPCION C__ESTADO,'
      '       P2.DESCRIPCION C__PAIS, '
      '       S.TEL_PRINCIPAL C_TELEFONO, '
      '       S.TELEFONOS C_TELEFONOS, '
      '       S.RFC C_RFC, '
      '       S.FACTURAR_A C_FACTURAR_A, '
      '       S.RAZON_SOCIAL C_RAZON_SOCIAL, '
      '       SC.FORMAS_DE_PAGO C_FORMA_DE_PAGO, '
      '       SC.MONEDA C_MONEDA,'
      '       SC.VENDEDOR C_VENDEDOR,'
      '       S4.NOMBRE C_V_NOMBRE,'
      '       SC.REMITENTE C_REMITENTE,'
      '       S5.NOMBRE C_R_NOMBRE,'
      '       SC.ESTATUS C_ESTATUS'
      
        'FROM (CLIENTES_EXTENSIONES CE LEFT JOIN SUJETOS_CLIENTES SC ON C' +
        'E.SUJETO_CLIENTE = SC.ID)'
      '     LEFT JOIN SUJETOS S                 ON S.ID   = SC.SUJETO'
      
        '     LEFT JOIN SUJETOS_TRANSPORTISTAS ST ON ST.ID  = CE.TRANSPOR' +
        'TISTA'
      '     LEFT JOIN SUJETOS S2                ON S2.ID  = ST.SUJETO'
      
        '     LEFT JOIN SUJETOS_REMITENTES SR     ON SR.ID  = CE.REMITENT' +
        'E'
      '     LEFT JOIN SUJETOS S3                ON S3.ID  = SR.SUJETO'
      '     LEFT JOIN SUJETOS_VENDEDORES SV     ON SV.ID  = SC.VENDEDOR'
      '     LEFT JOIN SUJETOS S4                ON S4.ID  = SV.SUJETO'
      
        '     LEFT JOIN DOMICILIOS D              ON D.ID   = CE.DIRECCIO' +
        'N'
      '     LEFT JOIN ENTIDAD_GEOGRAFICA C      ON C.ID   = D.CIUDAD '
      '     LEFT JOIN ENTIDAD_GEOGRAFICA E      ON E.ID   = C.LINK '
      '     LEFT JOIN ENTIDAD_GEOGRAFICA P      ON P.ID   = E.LINK'
      '     LEFT JOIN DOMICILIOS D2             ON D2.ID  = S.DOMICILIO'
      '     LEFT JOIN ENTIDAD_GEOGRAFICA C2     ON C2.ID  = D2.CIUDAD '
      '     LEFT JOIN ENTIDAD_GEOGRAFICA E2     ON E2.ID  = C2.LINK '
      '     LEFT JOIN ENTIDAD_GEOGRAFICA P2     ON P2.ID  = E2.LINK'
      
        '     LEFT JOIN SUJETOS_REMITENTES SR2    ON SR2.ID = SC.REMITENT' +
        'E'
      '     LEFT JOIN SUJETOS S5                ON S5.ID  = SR2.SUJETO')
    Left = 208
    Top = 128
    object qyBusquedaCE_ID: TIntegerField
      FieldName = 'CE_ID'
      Origin = 'CLIENTES_EXTENSIONES.ID'
      Required = True
    end
    object qyBusquedaCE_CLAVE: TIBStringField
      FieldName = 'CE_CLAVE'
      Origin = 'CLIENTES_EXTENSIONES.CLAVE'
    end
    object qyBusquedaCE_NOMBRE: TIBStringField
      FieldName = 'CE_NOMBRE'
      Origin = 'CLIENTES_EXTENSIONES.NOMBRE'
      Size = 100
    end
    object qyBusquedaCE_LIMITE_CREDITO: TFloatField
      FieldName = 'CE_LIMITE_CREDITO'
      Origin = 'CLIENTES_EXTENSIONES.LIMITE_CREDITO'
    end
    object qyBusquedaCE_REF_CLI_PROV: TIBStringField
      FieldName = 'CE_REF_CLI_PROV'
      Origin = 'CLIENTES_EXTENSIONES.PROVEEDOR'
    end
    object qyBusquedaCE_DIRECCION: TIntegerField
      FieldName = 'CE_DIRECCION'
      Origin = 'CLIENTES_EXTENSIONES.DIRECCION'
    end
    object qyBusquedaCE_CALLE: TIBStringField
      FieldName = 'CE_CALLE'
      Origin = 'DOMICILIOS.CALLE'
      Size = 100
    end
    object qyBusquedaCE_NUM_EXT: TIBStringField
      FieldName = 'CE_NUM_EXT'
      Origin = 'DOMICILIOS.NUM_EXT'
      Size = 10
    end
    object qyBusquedaCE_LETRA: TIBStringField
      FieldName = 'CE_LETRA'
      Origin = 'DOMICILIOS.LETRA'
      Size = 10
    end
    object qyBusquedaCE_NUM_INT: TIBStringField
      FieldName = 'CE_NUM_INT'
      Origin = 'DOMICILIOS.NUM_INT'
      Size = 10
    end
    object qyBusquedaCE_CP: TIBStringField
      FieldName = 'CE_CP'
      Origin = 'DOMICILIOS.CP'
      Size = 10
    end
    object qyBusquedaCE_COLONIA: TIBStringField
      FieldName = 'CE_COLONIA'
      Origin = 'DOMICILIOS.COLONIA'
      Size = 50
    end
    object qyBusquedaCE_CIUDAD: TIntegerField
      FieldName = 'CE_CIUDAD'
      Origin = 'DOMICILIOS.CIUDAD'
    end
    object qyBusquedaCE__CIUDAD: TIBStringField
      FieldName = 'CE__CIUDAD'
      Origin = 'ENTIDAD_GEOGRAFICA.DESCRIPCION'
      Size = 100
    end
    object qyBusquedaCE__ESTADO: TIBStringField
      FieldName = 'CE__ESTADO'
      Origin = 'ENTIDAD_GEOGRAFICA.DESCRIPCION'
      Size = 100
    end
    object qyBusquedaCE__PAIS: TIBStringField
      FieldName = 'CE__PAIS'
      Origin = 'ENTIDAD_GEOGRAFICA.DESCRIPCION'
      Size = 100
    end
    object qyBusquedaCE_TRANSPORTISTA: TIntegerField
      FieldName = 'CE_TRANSPORTISTA'
      Origin = 'CLIENTES_EXTENSIONES.TRANSPORTISTA'
    end
    object qyBusquedaCE_T_NOMBRE: TIBStringField
      FieldName = 'CE_T_NOMBRE'
      Origin = 'SUJETOS.NOMBRE'
      Size = 100
    end
    object qyBusquedaCE_RFC: TIBStringField
      FieldName = 'CE_RFC'
      Origin = 'CLIENTES_EXTENSIONES.RFC'
    end
    object qyBusquedaCE_FLAG_FACT: TIBStringField
      FieldName = 'CE_FLAG_FACT'
      Origin = 'CLIENTES_EXTENSIONES.FLAG_FACTURAR'
      FixedChar = True
      Size = 2
    end
    object qyBusquedaCE_REMITENTE: TIntegerField
      FieldName = 'CE_REMITENTE'
      Origin = 'CLIENTES_EXTENSIONES.REMITENTE'
    end
    object qyBusquedaCE_R_NOMBRE: TIBStringField
      FieldName = 'CE_R_NOMBRE'
      Origin = 'SUJETOS.NOMBRE'
      Size = 100
    end
    object qyBusquedaCE_ESTATUS: TIBStringField
      FieldName = 'CE_ESTATUS'
      Origin = 'CLIENTES_EXTENSIONES.ESTATUS'
    end
    object qyBusquedaC_CLIENTE: TIntegerField
      FieldName = 'C_CLIENTE'
      Origin = 'CLIENTES_EXTENSIONES.SUJETO_CLIENTE'
      Required = True
    end
    object qyBusquedaC_CLAVE: TIBStringField
      FieldName = 'C_CLAVE'
      Origin = 'SUJETOS.CLAVE'
      Size = 50
    end
    object qyBusquedaC_NOMBRE: TIBStringField
      FieldName = 'C_NOMBRE'
      Origin = 'SUJETOS.NOMBRE'
      Size = 100
    end
    object qyBusquedaC_LIMITE_CREDITO: TFloatField
      FieldName = 'C_LIMITE_CREDITO'
      Origin = 'SUJETOS_CLIENTES.LIMITE_CREDITO'
    end
    object qyBusquedaC_DESC_PORC: TFloatField
      FieldName = 'C_DESC_PORC'
      Origin = 'SUJETOS_CLIENTES.DESCUENTO_PORC'
    end
    object qyBusquedaC_DOMICILIO: TIntegerField
      FieldName = 'C_DOMICILIO'
      Origin = 'SUJETOS.DOMICILIO'
    end
    object qyBusquedaC_CALLE: TIBStringField
      FieldName = 'C_CALLE'
      Origin = 'DOMICILIOS.CALLE'
      Size = 100
    end
    object qyBusquedaC_NUM_EXT: TIBStringField
      FieldName = 'C_NUM_EXT'
      Origin = 'DOMICILIOS.NUM_EXT'
      Size = 10
    end
    object qyBusquedaC_LETRA: TIBStringField
      FieldName = 'C_LETRA'
      Origin = 'DOMICILIOS.LETRA'
      Size = 10
    end
    object qyBusquedaC_NUM_INT: TIBStringField
      FieldName = 'C_NUM_INT'
      Origin = 'DOMICILIOS.NUM_INT'
      Size = 10
    end
    object qyBusquedaC_CP: TIBStringField
      FieldName = 'C_CP'
      Origin = 'DOMICILIOS.CP'
      Size = 10
    end
    object qyBusquedaC_COLONIA: TIBStringField
      FieldName = 'C_COLONIA'
      Origin = 'DOMICILIOS.COLONIA'
      Size = 50
    end
    object qyBusquedaC_CIUDAD: TIntegerField
      FieldName = 'C_CIUDAD'
      Origin = 'DOMICILIOS.CIUDAD'
    end
    object qyBusquedaC__CIUDAD: TIBStringField
      FieldName = 'C__CIUDAD'
      Origin = 'ENTIDAD_GEOGRAFICA.DESCRIPCION'
      Size = 100
    end
    object qyBusquedaC__ESTADO: TIBStringField
      FieldName = 'C__ESTADO'
      Origin = 'ENTIDAD_GEOGRAFICA.DESCRIPCION'
      Size = 100
    end
    object qyBusquedaC__PAIS: TIBStringField
      FieldName = 'C__PAIS'
      Origin = 'ENTIDAD_GEOGRAFICA.DESCRIPCION'
      Size = 100
    end
    object qyBusquedaC_TELEFONO: TIBStringField
      FieldName = 'C_TELEFONO'
      Origin = 'SUJETOS.TEL_PRINCIPAL'
    end
    object qyBusquedaC_TELEFONOS: TIntegerField
      FieldName = 'C_TELEFONOS'
      Origin = 'SUJETOS.TELEFONOS'
    end
    object qyBusquedaC_RFC: TIBStringField
      FieldName = 'C_RFC'
      Origin = 'SUJETOS.RFC'
    end
    object qyBusquedaC_FACTURAR_A: TIBStringField
      FieldName = 'C_FACTURAR_A'
      Origin = 'SUJETOS.FACTURAR_A'
      Size = 12
    end
    object qyBusquedaC_RAZON_SOCIAL: TIBStringField
      FieldName = 'C_RAZON_SOCIAL'
      Origin = 'SUJETOS.RAZON_SOCIAL'
      Size = 100
    end
    object qyBusquedaC_FORMA_DE_PAGO: TIntegerField
      FieldName = 'C_FORMA_DE_PAGO'
      Origin = 'SUJETOS_CLIENTES.FORMAS_DE_PAGO'
    end
    object qyBusquedaC_MONEDA: TIntegerField
      FieldName = 'C_MONEDA'
      Origin = 'SUJETOS_CLIENTES.MONEDA'
    end
    object qyBusquedaC_VENDEDOR: TIntegerField
      FieldName = 'C_VENDEDOR'
      Origin = 'SUJETOS_CLIENTES.VENDEDOR'
    end
    object qyBusquedaC_V_NOMBRE: TIBStringField
      FieldName = 'C_V_NOMBRE'
      Origin = 'SUJETOS.NOMBRE'
      Size = 100
    end
    object qyBusquedaC_REMITENTE: TIntegerField
      FieldName = 'C_REMITENTE'
      Origin = 'SUJETOS_CLIENTES.REMITENTE'
    end
    object qyBusquedaC_R_NOMBRE: TIBStringField
      FieldName = 'C_R_NOMBRE'
      Origin = 'SUJETOS.NOMBRE'
      Size = 100
    end
    object qyBusquedaC_ESTATUS: TIBStringField
      FieldName = 'C_ESTATUS'
      Origin = 'SUJETOS_CLIENTES.ESTATUS'
    end
    object qyBusquedaCE_TELEFONO: TIBStringField
      FieldName = 'CE_TELEFONO'
      Origin = 'CLIENTES_EXTENSIONES.TELEFONO'
    end
  end
  object pmCampos: TPopupMenu
    Left = 256
    Top = 128
    object itemID: TMenuItem
      Caption = 'ID'
      OnClick = itemIDClick
    end
    object itemCLAVE: TMenuItem
      Caption = 'CLAVE'
      OnClick = itemCLAVEClick
    end
    object itemEXTENSION: TMenuItem
      Caption = 'EXTENSION'
      OnClick = itemEXTENSIONClick
    end
    object itemCLIENTE: TMenuItem
      Caption = 'CLIENTE'
      OnClick = itemCLIENTEClick
    end
    object itemVENDEDOR: TMenuItem
      Caption = 'VENDEDOR'
      OnClick = itemVENDEDORClick
    end
    object itemREMITENTE_EXT: TMenuItem
      Caption = 'REMITENTE EXTENSION'
      OnClick = itemREMITENTE_EXTClick
    end
    object item_REMITENTE_CLIENTE: TMenuItem
      Caption = 'REMITENTE CLIENTE'
      OnClick = item_REMITENTE_CLIENTEClick
    end
  end
end
