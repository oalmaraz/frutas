object frmEstatusDestinatarios: TfrmEstatusDestinatarios
  Left = 577
  Top = 234
  ActiveControl = edNombre
  Caption = 'Destinatario(s)'
  ClientHeight = 271
  ClientWidth = 591
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 8
    Top = 5
    Width = 45
    Height = 16
    Caption = 'Nombre'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label3: TLabel
    Left = 256
    Top = 5
    Width = 63
    Height = 16
    Caption = 'Parte Local'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label4: TLabel
    Left = 407
    Top = 25
    Width = 15
    Height = 19
    Caption = '@'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label5: TLabel
    Left = 425
    Top = 5
    Width = 46
    Height = 16
    Caption = 'Dominio'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object paBotones: TPanel
    Left = 0
    Top = 231
    Width = 591
    Height = 40
    Align = alBottom
    TabOrder = 4
    object buGuardar: TBitBtn
      Left = 430
      Top = 5
      Width = 75
      Height = 30
      Caption = '&Guardar'
      Glyph.Data = {
        
        0000FF0000FF0000FF0000FF0000FF0000FF0000FF0000FF0000}
      NumGlyphs = 2
      TabOrder = 0
      OnClick = buGuardarClick
    end
    object buSalir: TBitBtn
      Left = 508
      Top = 5
      Width = 75
      Height = 30
      Caption = '&Salir'
      Glyph.Data = {
        
        A7B7A7A7B7A7A7B7A6A6B6A6A6B8DBDBE1FFFFFFFFFFFFFFFFFF}
      ModalResult = 5
      TabOrder = 1
    end
  end
  object edNombre: TEdit
    Left = 8
    Top = 24
    Width = 242
    Height = 21
    TabOrder = 0
  end
  object edLocal: TEdit
    Left = 256
    Top = 24
    Width = 150
    Height = 21
    TabOrder = 1
  end
  object edDominio: TEdit
    Left = 424
    Top = 24
    Width = 153
    Height = 21
    TabOrder = 2
  end
  object dgPartidas: TDBGrid
    Left = 8
    Top = 56
    Width = 569
    Height = 161
    TabStop = False
    DataSource = dsPartidas
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
  end
  object dsPartidas: TDataSource
    DataSet = qyPartidas
    Left = 150
    Top = 103
  end
  object qyPartidas: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM TL_ESTATUS_PROCESOS_MAIL(:P_QUIEN)')
    Left = 118
    Top = 103
    ParamData = <
      item
        DataType = ftString
        Name = 'P_QUIEN'
        ParamType = ptUnknown
        Value = '0'
      end>
    object qyPartidasR_ID: TIntegerField
      FieldName = 'R_ID'
      Origin = 'TL_ESTATUS_PROCESOS_MAIL.R_ID'
    end
    object qyPartidasR_EPD: TIntegerField
      FieldName = 'R_EPD'
      Origin = 'TL_ESTATUS_PROCESOS_MAIL.R_EPD'
    end
    object qyPartidasR_NOMBRE: TIBStringField
      FieldName = 'R_NOMBRE'
      Origin = 'TL_ESTATUS_PROCESOS_MAIL.R_NOMBRE'
      Size = 100
    end
    object qyPartidasR_CORREO: TIBStringField
      FieldName = 'R_CORREO'
      Origin = 'TL_ESTATUS_PROCESOS_MAIL.R_CORREO'
      Size = 101
    end
  end
  object spA_ESTATUS_PROCESOS_MAIL: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'A_ESTATUS_PROCESOS_MAIL'
    Left = 120
    Top = 136
    ParamData = <
      item
        DataType = ftInteger
        Name = 'R_ID'
        ParamType = ptOutput
      end
      item
        DataType = ftInteger
        Name = 'P_QUIEN'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'P_NOMBRE'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'P_LOCAL'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'P_DOMINIO'
        ParamType = ptInput
      end>
  end
  object spE_ESTATUS_PROCESOS_MAIL: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'E_ESTATUS_PROCESOS_MAIL'
    Left = 152
    Top = 136
    ParamData = <
      item
        DataType = ftInteger
        Name = 'R_ID'
        ParamType = ptOutput
      end
      item
        DataType = ftInteger
        Name = 'P_ID'
        ParamType = ptInput
        Value = '0'
      end>
  end
end
