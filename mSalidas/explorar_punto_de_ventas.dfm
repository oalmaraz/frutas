inherited frmExplorar_punto_de_ventas: TfrmExplorar_punto_de_ventas
  Left = 205
  Top = 143
  Caption = 'Punto de Ventas'
  ClientHeight = 537
  ClientWidth = 1020
  PopupMenu = pmPVCobrar
  OnShow = FormShow
  ExplicitWidth = 1036
  ExplicitHeight = 576
  PixelsPerInch = 96
  TextHeight = 13
  inherited paExplorarBusqueda: TPanel [0]
    Width = 1020
    Height = 48
    TabOrder = 2
    ExplicitWidth = 1020
    ExplicitHeight = 48
    object Shape9: TShape [0]
      Left = 389
      Top = 4
      Width = 68
      Height = 18
      Brush.Color = 3523308
      Pen.Color = 11764252
      Shape = stRoundRect
    end
    object Shape7: TShape [1]
      Left = 552
      Top = 4
      Width = 68
      Height = 18
      Brush.Color = 3523308
      Pen.Color = 11764252
      Shape = stRoundRect
    end
    object Shape1: TShape [2]
      Left = 6
      Top = 25
      Width = 83
      Height = 18
      Brush.Color = 14789952
      Pen.Color = 11764252
      Shape = stRoundRect
    end
    object Shape3: TShape [3]
      Left = 6
      Top = 4
      Width = 83
      Height = 18
      Brush.Color = 14789952
      Pen.Color = 11764252
      Shape = stRoundRect
    end
    inherited laCampo: TLabel
      Height = 48
      Visible = False
    end
    object Label24: TLabel [5]
      Left = 29
      Top = 6
      Width = 36
      Height = 13
      Caption = 'Usuario'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label10: TLabel [6]
      Left = 27
      Top = 27
      Width = 40
      Height = 13
      Caption = 'Almacen'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Shape4: TShape [7]
      Left = 91
      Top = 12
      Width = 14
      Height = 3
      Brush.Color = 11106843
      Pen.Color = 14789952
    end
    object Shape17: TShape [8]
      Left = 91
      Top = 33
      Width = 14
      Height = 3
      Brush.Color = 11106843
      Pen.Color = 14789952
    end
    object Label12: TLabel [9]
      Left = 226
      Top = 27
      Width = 54
      Height = 13
      Caption = 'F. Historico'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Shape6: TShape [10]
      Left = 289
      Top = 33
      Width = 14
      Height = 3
      Brush.Color = 11106843
      Pen.Color = 14789952
    end
    object Label4: TLabel [11]
      Left = 409
      Top = 6
      Width = 29
      Height = 13
      Caption = 'Fecha'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Shape10: TShape [12]
      Left = 459
      Top = 12
      Width = 14
      Height = 3
      Brush.Color = 11106843
      Pen.Color = 14789952
    end
    object Label3: TLabel [13]
      Left = 575
      Top = 6
      Width = 22
      Height = 13
      Caption = 'Caja'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Shape8: TShape [14]
      Left = 622
      Top = 12
      Width = 14
      Height = 3
      Brush.Color = 11106843
      Pen.Color = 14789952
    end
    object Shape15: TShape [15]
      Left = 622
      Top = 33
      Width = 14
      Height = 3
      Brush.Color = 11106843
      Pen.Color = 14789952
    end
    object Shape14: TShape [16]
      Left = 552
      Top = 25
      Width = 68
      Height = 18
      Brush.Color = 3523308
      Pen.Color = 11764252
      Shape = stRoundRect
    end
    object Label1: TLabel [17]
      Left = 563
      Top = 27
      Width = 47
      Height = 13
      Caption = 'Folio Caja'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label2: TLabel [18]
      Left = 813
      Top = 0
      Width = 80
      Height = 13
      Caption = 'Codigo Barras'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    inherited edExplorarValor: TEdit
      Left = 765
      Top = 13
      Width = 177
      Font.Charset = ANSI_CHARSET
      Font.Height = -21
      Font.Name = 'Tahoma'
      ParentFont = False
      ExplicitLeft = 765
      ExplicitTop = 13
      ExplicitWidth = 177
    end
    inherited edExplorarDel: TDateTimePicker
      TabOrder = 7
    end
    inherited edExplorarAl: TDateTimePicker
      TabOrder = 8
    end
    inherited buExpHoy: TBitBtn
      Left = 475
      Top = 3
      Width = 71
      Height = 21
      TabOrder = 9
      ExplicitLeft = 475
      ExplicitTop = 3
      ExplicitWidth = 71
      ExplicitHeight = 21
    end
    object deAlmacen: TDBEdit
      Left = 106
      Top = 24
      Width = 111
      Height = 21
      TabStop = False
      Color = 15788249
      DataField = 'R_ALMACEN_CLAVE'
      DataSource = dsVer_Datos_Cajero
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
    object deUsuario: TDBEdit
      Left = 106
      Top = 3
      Width = 279
      Height = 21
      TabStop = False
      Color = 15788249
      DataField = 'R_NOMBRE_USUARIO'
      DataSource = dsVer_Datos_Cajero
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
    end
    object deFHistorico: TDBEdit
      Left = 304
      Top = 24
      Width = 81
      Height = 21
      TabStop = False
      Color = 15788249
      DataField = 'R_FOLIO_HISTORICO'
      DataSource = dsFuente
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
    end
    object edFecha: TEdit
      Left = 475
      Top = 3
      Width = 71
      Height = 21
      TabStop = False
      Color = 15788249
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 4
      Text = '03/12/2001'
    end
    object deCaja: TDBEdit
      Left = 637
      Top = 3
      Width = 110
      Height = 21
      TabStop = False
      Color = 15788249
      DataField = 'R_NOMBRE'
      DataSource = dsVer_Datos_Cajero
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
    end
    object deFolio: TDBEdit
      Left = 637
      Top = 24
      Width = 110
      Height = 21
      TabStop = False
      Color = 15788249
      DataField = 'R_FOLIO_DE_VENTA'
      DataSource = dsFuente
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
    end
  end
  inherited dgExplorar: TDBGrid [1]
    Top = 110
    Width = 1020
    Height = 405
    Font.Charset = ANSI_CHARSET
    Font.Height = -16
    Font.Name = 'Tahoma'
    ParentFont = False
    TabOrder = 3
    OnKeyDown = dgExplorarKeyDown
  end
  inherited fpPadre: TPanel [2]
    Width = 1020
    ExplicitWidth = 1020
    object Label11: TLabel [0]
      Left = 776
      Top = -1
      Width = 150
      Height = 19
      Alignment = taCenter
      Caption = 'Cambio Ultima Venta'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Transparent = True
      Layout = tlCenter
    end
    inherited Panel1: TPanel
      Width = 1020
      TabOrder = 12
      ExplicitWidth = 1020
    end
    inherited ibVer: TBitBtn
      TabOrder = 0
    end
    inherited ibConstructor: TBitBtn
      Left = 900
      TabOrder = 1
      Visible = False
      ExplicitLeft = 900
    end
    inherited ibImprimir: TBitBtn
      Left = 234
      Caption = 'Corte'
      Glyph.Data = {
        
        00FFC6C3C6C6C3C6C6C3C6C6C3C6C6C3C60000FF0000FF0000FF}
      NumGlyphs = 2
      TabOrder = 2
      ExplicitLeft = 234
    end
    inherited ibPantalla: TBitBtn
      Left = 157
      Hint = 'Afectar'
      Caption = 'Afectar'
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C00000}
      NumGlyphs = 2
      TabOrder = 3
      ExplicitLeft = 157
    end
    inherited ibExportarXLS: TBitBtn
      Left = 533
      TabOrder = 4
      ExplicitLeft = 533
    end
    inherited ibExportarHTML: TBitBtn
      Left = 610
      TabOrder = 5
      ExplicitLeft = 610
    end
    inherited ibExportarXML: TBitBtn
      Left = 687
      TabOrder = 11
      ExplicitLeft = 687
    end
    object ibReImpresionFactura: TBitBtn
      Left = 379
      Top = 23
      Width = 75
      Height = 34
      Cursor = crHandPoint
      Hint = 'Re-Impresion'
      Caption = 'Re-Fact'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FF0000FF0000FF0000FF0000FF0000FF0000FF0000FF0000FF}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnClick = ibReImpresionFacturaClick
    end
    object ibReImpNV: TBitBtn
      Left = 456
      Top = 23
      Width = 75
      Height = 34
      Cursor = crHandPoint
      Hint = 'Re-Impresion'
      Caption = 'Re-NV'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FF0000FF0000FF0000FF0000FF0000FF0000FF0000FF0000FF}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 8
      OnClick = ibReImpNVClick
    end
    object ibF10: TBitBtn
      Left = 80
      Top = 23
      Width = 75
      Height = 34
      Cursor = crHandPoint
      Hint = 'cancelar'
      Caption = 'cancelar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0}
      NumGlyphs = 2
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 7
      OnClick = ibF10Click
    end
    object edCambio: TEdit
      Left = 765
      Top = 21
      Width = 172
      Height = 37
      TabStop = False
      CharCase = ecUpperCase
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -24
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 9
      Text = '0.00'
    end
    object buVentas: TBitBtn
      Left = 311
      Top = 23
      Width = 66
      Height = 34
      Cursor = crHandPoint
      Hint = 'Imprimir'
      Caption = 'Ventas'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      NumGlyphs = 2
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 10
      OnClick = buVentasClick
    end
  end
  inherited Panel2: TPanel [3]
    Top = 515
    Width = 1020
    ExplicitTop = 515
    ExplicitWidth = 1020
  end
  inherited qyExplorar: TIBQuery
    SQL.Strings = (
      'SELECT *'
      'FROM PVENTAS_ENCABEZADO2(:P_ALMACEN)')
    ParamData = <
      item
        DataType = ftString
        Name = 'P_ALMACEN'
        ParamType = ptUnknown
        Value = '0'
      end>
    object qyExplorarR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyExplorarR_FOLIO_DE_VENTA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_FOLIO_DE_VENTA'
      ReadOnly = True
    end
    object qyExplorarR_SUBTOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_SUBTOTAL'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_IVA: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IVA'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_IEPS: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IEPS'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_IMPUESTO1: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO1'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_IMPUESTO2: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO2'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_TOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_TOTAL'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_DESCUENTO_GLOBAL_PORC: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCUENTO_GLOBAL_PORC'
      ReadOnly = True
    end
    object qyExplorarR_DESCUENTO_CANTIDAD_GLOBAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCUENTO_CANTIDAD_GLOBAL'
      ReadOnly = True
    end
    object qyExplorarR_DESCUENTO_CANTIDAD_PARCIAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCUENTO_CANTIDAD_PARCIAL'
      ReadOnly = True
    end
    object qyExplorarR_GRANTOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_GRANTOTAL'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_ESTATUS: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ESTATUS'
      ReadOnly = True
    end
    object qyExplorarR_CONDICION_COMERCIAL: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CONDICION_COMERCIAL'
      ReadOnly = True
    end
    object qyExplorarR_CAMBIO_ENTREGADO: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAMBIO_ENTREGADO'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_TOTAL_REF_PAGOS: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_TOTAL_REF_PAGOS'
      ReadOnly = True
    end
    object qyExplorarR_SALDO: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_SALDO'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_CLIENTE: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE'
      ReadOnly = True
    end
    object qyExplorarR_NOMBRE_CLIENTE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_NOMBRE_CLIENTE'
      ReadOnly = True
      Size = 100
    end
    object qyExplorarR_EXTENSION: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_EXTENSION'
      ReadOnly = True
    end
    object qyExplorarR_NOMBRE_EXTENSION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_NOMBRE_EXTENSION'
      ReadOnly = True
      Size = 100
    end
    object qyExplorarR_MONEDA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_MONEDA'
      ReadOnly = True
    end
    object qyExplorarR_FOLIO_HISTORICO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_FOLIO_HISTORICO'
      ReadOnly = True
    end
    object qyExplorarR_LIMITE_CREDITO: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_LIMITE_CREDITO'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_LIMITE_DISPONIBLE: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_LIMITE_DISPONIBLE'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyExplorarR_CAJA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJA'
      ReadOnly = True
    end
    object qyExplorarR_CAJA_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJA_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyExplorarR_FECHA_ALTA: TDateTimeField
      FieldKind = fkInternalCalc
      FieldName = 'R_FECHA_ALTA'
      ReadOnly = True
      DisplayFormat = 'dd/mm/yyyy hh:mm:ss am/pm'
    end
    object qyExplorarR_MONEDA_CLAVE: TIBStringField
      FieldName = 'R_MONEDA_CLAVE'
      Origin = 'PVENTAS_ENCABEZADO2.R_MONEDA_CLAVE'
      Size = 10
    end
    object qyExplorarR_MONEDA_DESCRIPCION: TIBStringField
      FieldName = 'R_MONEDA_DESCRIPCION'
      Origin = 'PVENTAS_ENCABEZADO2.R_MONEDA_DESCRIPCION'
      Size = 50
    end
    object qyExplorarR_FOLIO_ALMACEN: TIntegerField
      FieldName = 'R_FOLIO_ALMACEN'
      Origin = 'PVENTAS_ENCABEZADO2.R_FOLIO_ALMACEN'
    end
    object qyExplorarR_FOLIO_GENERAL: TIntegerField
      FieldName = 'R_FOLIO_GENERAL'
      Origin = 'PVENTAS_ENCABEZADO2.R_FOLIO_GENERAL'
    end
    object qyExplorarR_CLIENTE_C: TIBStringField
      FieldName = 'R_CLIENTE_C'
      Origin = 'PVENTAS_ENCABEZADO2.R_CLIENTE_C'
      Size = 50
    end
    object qyExplorarR_CLIENTE_T: TIBStringField
      FieldName = 'R_CLIENTE_T'
      Origin = 'PVENTAS_ENCABEZADO2.R_CLIENTE_T'
    end
  end
  object iePagos: TIBEvents
    AutoRegister = False
    Database = dmIBModulo.ibSistema
    Registered = False
    OnEventAlert = iePagosEventAlert
    Left = 83
    Top = 321
  end
  object spACT_CLIENTE_PVTA: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'ACT_CLIENTE_PVTA'
    Left = 24
    Top = 320
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_ID'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_EXTENSION'
        ParamType = ptInput
      end>
  end
  object spNR2: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'NR2'
    Left = 112
    Top = 321
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_ID'
        ParamType = ptInput
        Value = '0'
      end>
  end
  object pmPVCobrar: TPopupMenu
    OwnerDraw = True
    Left = 144
    Top = 320
    object itemF10: TMenuItem
      Caption = 'Cancelar'
      ShortCut = 121
    end
    object itemF12: TMenuItem
      Caption = 'Afectar'
      ShortCut = 123
      OnClick = itemF12Click
    end
  end
  object qyVer_Datos_Cajero: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM VER_DATOS_CAJERO(:P_USUARIO, "Ventas")')
    Left = 248
    Top = 348
    ParamData = <
      item
        DataType = ftString
        Name = 'P_USUARIO'
        ParamType = ptUnknown
        Value = '1'
      end>
    object qyVer_Datos_CajeroR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyVer_Datos_CajeroR_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyVer_Datos_CajeroR_ALMACEN: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN'
      ReadOnly = True
    end
    object qyVer_Datos_CajeroR_ALMACEN_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN_CLAVE'
      ReadOnly = True
    end
    object qyVer_Datos_CajeroR_ALMACEN_OBSERVACIONES: TBlobField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN_OBSERVACIONES'
      ReadOnly = True
      Size = 8
    end
    object qyVer_Datos_CajeroR_USUARIO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_USUARIO'
      ReadOnly = True
    end
    object qyVer_Datos_CajeroR_NOMBRE_USUARIO: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_NOMBRE_USUARIO'
      ReadOnly = True
      Size = 100
    end
    object qyVer_Datos_CajeroR_BAJA: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_BAJA'
      ReadOnly = True
      Size = 2
    end
    object qyVer_Datos_CajeroR_ESTATUS: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ESTATUS'
      ReadOnly = True
    end
    object qyVer_Datos_CajeroR_REGISTROS: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_REGISTROS'
      ReadOnly = True
    end
    object qyVer_Datos_CajeroR_MODULO_FUNCION: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_MODULO_FUNCION'
      ReadOnly = True
    end
    object qyVer_Datos_CajeroR_MONEDA: TIntegerField
      FieldName = 'R_MONEDA'
      Origin = 'VER_DATOS_CAJERO.R_MONEDA'
    end
  end
  object dsVer_Datos_Cajero: TDataSource
    DataSet = qyVer_Datos_Cajero
    Left = 280
    Top = 348
  end
  object qyEstatus: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    DataSource = dsFuente
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM VIEW_ESTATUS(34, :P_ESTATUS)')
    Left = 538
    Top = 336
    ParamData = <
      item
        DataType = ftString
        Name = 'P_ESTATUS'
        ParamType = ptUnknown
        Value = 'Sin Afectar'
      end>
  end
  object dsEstatus: TDataSource
    DataSet = qyEstatus
    Left = 570
    Top = 336
  end
  object spEstatus: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    Left = 505
    Top = 336
  end
  object spT_MSJ_LIMITE_CREDITO: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'T_MSJ_LIMITE_CREDITO'
    Left = 640
    Top = 336
    ParamData = <
      item
        DataType = ftString
        Name = 'R_DECIR_MSJ'
        ParamType = ptOutput
      end
      item
        DataType = ftString
        Name = 'R_MSJ'
        ParamType = ptOutput
      end
      item
        DataType = ftFloat
        Name = 'R_LIMITE'
        ParamType = ptOutput
      end
      item
        DataType = ftFloat
        Name = 'R_CONTADO'
        ParamType = ptOutput
      end
      item
        DataType = ftFloat
        Name = 'R_CREDITO'
        ParamType = ptOutput
      end
      item
        DataType = ftInteger
        Name = 'P_ID'
        ParamType = ptInput
      end>
  end
end
