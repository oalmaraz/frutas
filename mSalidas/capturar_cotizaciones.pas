...
uses
  ...
  capturarMDI, Db, IBQuery,   explorar_cotizaciones_estatus, 
  Menus, UfrmFrameMaterial, Grids, DBGrids,  
  StdCtrls, DBCtrls,  Buttons, IBStoredProc, IBTable,
  IBCustomDataSet, Mask, ExtCtrls;
...
begin
   ...
   spADD_PARTIDA_COTIZACION.ParamByName('P_CANTIDAD').AsFloat    := StrToFloat(MarcoMateriales1.edCantidad.Text);
   spADD_PARTIDA_COTIZACION.ParamByName('P_PRECIO').AsFloat      := StrToFloat(MarcoMateriales1.edPrecioCosto.Text);
   ...
   spADD_PARTIDA_COTIZACION.ParamByName('P_PORC_DP').AsFloat     := StrToFloat(MarcoMateriales1.edDescuentoPorc.Text);
...
procedure TfrmCapturar_cotizaciones.cbEstatusChange(Sender: TObject);
begin
  inherited;
