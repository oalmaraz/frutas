...
uses
  ...
  Buttons, ComCtrls, IBStoredProc, IBTable, IBCustomDataSet, IBQuery, Mask,
  System.ImageList;
...
begin
   ...
   if deFechaAlta.Focused and deClave.Enabled and deClave.Visible then
      deClave.SetFocus;
   entero   := Int(StrToDate(deFechaAlta.Text));
   fraccion := Frac(StrToDate(deFechaAlta.Text));
   if entero > 0 then
   begin
      if fraccion = 0 then
   ...
   if deFEntrega.Focused and deClave.Enabled and deClave.Visible then
      deClave.SetFocus;
   entero   := Int(StrToDate(deFEntrega.Text));
   fraccion := Frac(StrToDate(deFEntrega.Text));
   if entero > 0 then
   begin
      if fraccion = 0 then
         dsFuente.DataSet.FieldByName('FECHA_ENTREGA').AsDateTime := StrToDate(deFEntrega.Text) + time;
   end;
   //validacion fecha entrega real
   if deFEntregaReal.Focused and deClave.Enabled and deClave.Visible then
      deClave.SetFocus;
   entero   := Int(StrToDate(deFEntregaReal.Text));
   fraccion := Frac(StrToDate(deFEntregaReal.Text));
   if entero > 0 then
   begin
      if fraccion = 0 then
         dsFuente.DataSet.FieldByName('FECHA_ENTREGA_REAL').AsDateTime := StrToDate(deFEntregaReal.Text) + time;
   end;
   //validacion fecha validez
   if deFechaValidez.Focused and deClave.Enabled and deClave.Visible then
      deClave.SetFocus;
   entero   := Int(StrToDate(deFechaValidez.Text));
   fraccion := Frac(StrToDate(deFechaValidez.Text));
   if entero > 0 then
   begin
      if fraccion = 0 then
         dsFuente.DataSet.FieldByName('FECHA_VALIDEZ').AsDateTime := StrToDate(deFechaValidez.Text) + time;
   end;
end;
