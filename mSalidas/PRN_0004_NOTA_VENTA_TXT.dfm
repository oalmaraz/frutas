inherited frmPRN_0004_TXT: TfrmPRN_0004_TXT
  Left = 238
  Top = 51
  ActiveControl = ceConsecutivo
  Caption = 'PRN_0004'
  ClientHeight = 356
  ClientWidth = 781
  OldCreateOrder = True
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  inherited fpPadre: TPanel
    Width = 781
    ExplicitWidth = 781
    object Label3: TLabel [0]
      Left = 5
      Top = 22
      Width = 30
      Height = 13
      Caption = 'Fecha'
    end
    object Label1: TLabel [1]
      Left = 100
      Top = 22
      Width = 69
      Height = 13
      Caption = 'Nota de Venta'
    end
    inherited Panel1: TPanel
      Width = 781
      TabOrder = 5
      ExplicitWidth = 781
    end
    object deFecha: TDateTimePicker
      Left = 5
      Top = 37
      Width = 91
      Height = 21
      Date = 42124.553508020800000000
      Time = 42124.553508020800000000
      TabOrder = 0
    end
    object ceConsecutivo: TEdit
      Left = 100
      Top = 37
      Width = 92
      Height = 21
      TabOrder = 1
      OnKeyPress = ceConsecutivoKeyPress
    end
    object ibBusqueda: TBitBtn
      Left = 196
      Top = 23
      Width = 57
      Height = 38
      Caption = '&Buscar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        0000000000000000000000000000000000000000000000FFFFFF}
      Layout = blGlyphTop
      ParentFont = False
      ParentShowHint = False
      ShowHint = False
      TabOrder = 2
      OnClick = ibBusquedaClick
    end
    object ibImprimir: TBitBtn
      Left = 256
      Top = 23
      Width = 57
      Height = 38
      Hint = 'Imprimir'
      Caption = '&Impresion'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        D8D7E6D8D7E6D8D7E6D8D7E6D8D7E6D8D7E6D8D7FFFFFFFFFFFF}
      Layout = blGlyphTop
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = ibImprimirClick
    end
    object ibCerrar: TBitBtn
      Left = 316
      Top = 23
      Width = 37
      Height = 38
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphRight
      ModalResult = 2
      ParentFont = False
      ParentShowHint = False
      ShowHint = False
      TabOrder = 4
    end
  end
  object dgFacturados: TDBGrid
    Left = 0
    Top = 69
    Width = 781
    Height = 287
    Align = alClient
    DataSource = dsBusquedaVenta
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnDblClick = dgFacturadosDblClick
  end
  object dsBusquedaVenta: TDataSource
    DataSet = qyBusquedaVenta
    OnDataChange = dsBusquedaVentaDataChange
    Left = 40
    Top = 120
  end
  object qyBusquedaVenta: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM VER_FOLIOS_VTA(:P_FECHA ,'
      '                                          :P_FOLIO,'
      '                                          :P_ALMACEN)'
      '')
    Left = 8
    Top = 120
    ParamData = <
      item
        DataType = ftString
        Name = 'P_FECHA'
        ParamType = ptUnknown
        Value = '26/09/2002'
      end
      item
        DataType = ftUnknown
        Name = 'P_FOLIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'P_ALMACEN'
        ParamType = ptUnknown
      end>
    object qyBusquedaVentaR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyBusquedaVentaR_FOLIO_DE_VENTA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_FOLIO_DE_VENTA'
      ReadOnly = True
    end
    object qyBusquedaVentaR_ALMACEN: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN'
      ReadOnly = True
    end
    object qyBusquedaVentaR_ALMACEN_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyBusquedaVentaR_CAJERO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJERO'
      ReadOnly = True
    end
    object qyBusquedaVentaR_CAJERO_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJERO_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyBusquedaVentaR_CONDICION_COMERCIAL: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CONDICION_COMERCIAL'
      ReadOnly = True
    end
    object qyBusquedaVentaR_CONDICION_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CONDICION_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyBusquedaVentaR_CLIENTE: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE'
      ReadOnly = True
    end
    object qyBusquedaVentaR_CLIENTE_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyBusquedaVentaR_EXTENSION: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_EXTENSION'
      ReadOnly = True
    end
    object qyBusquedaVentaR_EXTENSION_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_EXTENSION_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyBusquedaVentaR_FECHA_ALTA: TDateTimeField
      FieldKind = fkInternalCalc
      FieldName = 'R_FECHA_ALTA'
      ReadOnly = True
      DisplayFormat = 'dd/mmm/yyyy'
    end
    object qyBusquedaVentaR_CAJA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJA'
      ReadOnly = True
    end
    object qyBusquedaVentaR_CAJA_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJA_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyBusquedaVentaR_GRANTOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_GRANTOTAL'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyBusquedaVentaR_MONEDA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_MONEDA'
      ReadOnly = True
    end
    object qyBusquedaVentaR_MONEDA_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_MONEDA_DESCRIPCION'
      ReadOnly = True
      Size = 50
    end
  end
  object qyEmpresa: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM VER_EMPRESA')
    Left = 72
    Top = 151
    object qyEmpresaR_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLAVE'
      ReadOnly = True
      Size = 50
    end
    object qyEmpresaR_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyEmpresaR_RAZON_SOCIAL: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_RAZON_SOCIAL'
      ReadOnly = True
      Size = 100
    end
    object qyEmpresaR_TELEFONO: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_TELEFONO'
      ReadOnly = True
    end
    object qyEmpresaR_RFC: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_RFC'
      ReadOnly = True
    end
    object qyEmpresaR_CURP: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CURP'
      ReadOnly = True
    end
    object qyEmpresaR_CIUDAD_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CIUDAD_DESCRIPCION'
      ReadOnly = True
      Size = 304
    end
  end
  object dsEmpresa: TDataSource
    DataSet = qyEmpresa
    Left = 104
    Top = 151
  end
  object qyEncabezadoNV: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    OnCalcFields = qyEncabezadoNVCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM PEDIDO_PUNTO_VENTA_ENC(:P_VENTA, '#39#39')')
    Left = 8
    Top = 181
    ParamData = <
      item
        DataType = ftString
        Name = 'P_VENTA'
        ParamType = ptUnknown
        Value = '404'
      end>
    object qyEncabezadoNVR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyEncabezadoNVR_FOLIO_DE_VENTA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_FOLIO_DE_VENTA'
      ReadOnly = True
    end
    object qyEncabezadoNVR_ALMACEN: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN'
      ReadOnly = True
    end
    object qyEncabezadoNVR_CAJERO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJERO'
      ReadOnly = True
    end
    object qyEncabezadoNVR_CONDICION_COMERCIAL: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CONDICION_COMERCIAL'
      ReadOnly = True
    end
    object qyEncabezadoNVR_CLIENTE: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE'
      ReadOnly = True
    end
    object qyEncabezadoNVR_EXTENSION: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_EXTENSION'
      ReadOnly = True
    end
    object qyEncabezadoNVR_FECHA_ALTA: TDateTimeField
      FieldKind = fkInternalCalc
      FieldName = 'R_FECHA_ALTA'
      ReadOnly = True
    end
    object qyEncabezadoNVR_CAJA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJA'
      ReadOnly = True
    end
    object qyEncabezadoNVR_SUBTOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_SUBTOTAL'
      ReadOnly = True
    end
    object qyEncabezadoNVR_IVA: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IVA'
      ReadOnly = True
    end
    object qyEncabezadoNVR_IEPS: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IEPS'
      ReadOnly = True
    end
    object qyEncabezadoNVR_IMPUESTO1: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO1'
      ReadOnly = True
    end
    object qyEncabezadoNVR_IMPUESTO2: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO2'
      ReadOnly = True
    end
    object qyEncabezadoNVR_TOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_TOTAL'
      ReadOnly = True
    end
    object qyEncabezadoNVR_DESCUENTO_GLOBAL_PORC: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCUENTO_GLOBAL_PORC'
      ReadOnly = True
    end
    object qyEncabezadoNVR_DESCUENTO_CANTIDAD_GLOBAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCUENTO_CANTIDAD_GLOBAL'
      ReadOnly = True
    end
    object qyEncabezadoNVR_DESCUENTO_CANTIDAD_PARCIAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCUENTO_CANTIDAD_PARCIAL'
      ReadOnly = True
    end
    object qyEncabezadoNVR_GRANTOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_GRANTOTAL'
      ReadOnly = True
    end
    object qyEncabezadoNVR_MONEDA: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_MONEDA'
      ReadOnly = True
    end
    object qyEncabezadoNVR_CAMBIO_ENTREGADO: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAMBIO_ENTREGADO'
      ReadOnly = True
    end
    object qyEncabezadoNVR_FOLIO_HISTORICO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_FOLIO_HISTORICO'
      ReadOnly = True
    end
    object qyEncabezadoNVR_FOLIO_DEVOLUCION: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_FOLIO_DEVOLUCION'
      ReadOnly = True
    end
    object qyEncabezadoNVR_ESTATUS: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ESTATUS'
      ReadOnly = True
    end
    object qyEncabezadoNVR_TOTAL_REF_PAGOS: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_TOTAL_REF_PAGOS'
      ReadOnly = True
    end
    object qyEncabezadoNVR_ALMACEN_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN_CLAVE'
      ReadOnly = True
    end
    object qyEncabezadoNVR_ALMACEN_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyEncabezadoNVR_CAJERO_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJERO_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyEncabezadoNVR_CONDICION_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CONDICION_CLAVE'
      ReadOnly = True
    end
    object qyEncabezadoNVR_CONDICION_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CONDICION_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyEncabezadoNVR_CLIENTE_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyEncabezadoNVR_EXTENSION_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_EXTENSION_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyEncabezadoNVR_CAJA_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJA_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyEncabezadoNVR_MONEDA_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_MONEDA_CLAVE'
      ReadOnly = True
      Size = 10
    end
    object qyEncabezadoNVR_MONEDA_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_MONEDA_DESCRIPCION'
      ReadOnly = True
      Size = 50
    end
    object qyEncabezadoNVR_CLIENTE_CIUDAD: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE_CIUDAD'
      ReadOnly = True
      Size = 304
    end
    object qyEncabezadoNVR_ALMACEN_SERIE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN_SERIE'
      ReadOnly = True
      Size = 10
    end
    object qyEncabezadoNVR_ALMACEN_OBSERVACIONES: TBlobField
      FieldKind = fkInternalCalc
      FieldName = 'R_ALMACEN_OBSERVACIONES'
      ReadOnly = True
      Size = 8
    end
    object qyEncabezadoNVR_CAJERO_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJERO_CLAVE'
      ReadOnly = True
    end
    object qyEncabezadoNVR_CAJAS: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJAS'
      ReadOnly = True
    end
    object qyEncabezadoNVR_TOTAL_CAJAS: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_TOTAL_CAJAS'
      ReadOnly = True
    end
    object qyEncabezadoNVR_CLIENTE_DOMICILIO: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE_DOMICILIO'
      ReadOnly = True
      Size = 122
    end
    object qyEncabezadoNVR_CLIENTE_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE_CLAVE'
      ReadOnly = True
      Size = 50
    end
    object qyEncabezadoNVR_CLIENTE_RFC: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE_RFC'
      ReadOnly = True
    end
    object qyEncabezadoNVR_FECHA_VENCIMIENTO: TDateTimeField
      FieldKind = fkInternalCalc
      FieldName = 'R_FECHA_VENCIMIENTO'
      ReadOnly = True
    end
    object qyEncabezadoNVR_PRIORIDAD: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_PRIORIDAD'
      ReadOnly = True
      Size = 2
    end
    object qyEncabezadoNVR_SERIE_NV: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_SERIE_NV'
      ReadOnly = True
      Size = 10
    end
    object qyEncabezadoNVR_CONSECUTIVO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CONSECUTIVO'
      ReadOnly = True
    end
    object qyEncabezadoNVcantidadconletra: TStringField
      FieldKind = fkCalculated
      FieldName = 'cantidadconletra'
      Size = 100
      Calculated = True
    end
    object qyEncabezadoNVtotal: TFloatField
      FieldKind = fkCalculated
      FieldName = 'total'
      Calculated = True
    end
  end
  object dsEncabezado: TDataSource
    DataSet = qyEncabezadoNV
    Left = 40
    Top = 181
  end
  object dsDetalle: TDataSource
    DataSet = qyDetalleNV
    Left = 40
    Top = 212
  end
  object qyDetalleNV: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    OnCalcFields = qyDetalleNVCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM PEDIDO_PUNTO_VENTA_DET(:P_VENTA)')
    Left = 8
    Top = 212
    ParamData = <
      item
        DataType = ftString
        Name = 'P_VENTA'
        ParamType = ptUnknown
        Value = '404'
      end>
    object qyDetalleNVR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyDetalleNVR_PUNTO_DE_VENTA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_PUNTO_DE_VENTA'
      ReadOnly = True
    end
    object qyDetalleNVR_MATERIAL: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_MATERIAL'
      ReadOnly = True
    end
    object qyDetalleNVR_CANTIDAD: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_CANTIDAD'
      ReadOnly = True
    end
    object qyDetalleNVR_UNIDAD: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_UNIDAD'
      ReadOnly = True
    end
    object qyDetalleNVR_PRECIO_VENTA: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_PRECIO_VENTA'
      ReadOnly = True
    end
    object qyDetalleNVR_COSTO_COMPRA: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_COSTO_COMPRA'
      ReadOnly = True
    end
    object qyDetalleNVR_VENDEDOR: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_VENDEDOR'
      ReadOnly = True
    end
    object qyDetalleNVR_DESCUENTO_PORC: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCUENTO_PORC'
      ReadOnly = True
    end
    object qyDetalleNVR_SUBTOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_SUBTOTAL'
      ReadOnly = True
    end
    object qyDetalleNVR_IVA: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IVA'
      ReadOnly = True
    end
    object qyDetalleNVR_IEPS: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IEPS'
      ReadOnly = True
    end
    object qyDetalleNVR_IMPUESTO1: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO1'
      ReadOnly = True
    end
    object qyDetalleNVR_IMPUESTO2: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO2'
      ReadOnly = True
    end
    object qyDetalleNVR_IVA_PORC: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IVA_PORC'
      ReadOnly = True
    end
    object qyDetalleNVR_IEPS_PORC: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IEPS_PORC'
      ReadOnly = True
    end
    object qyDetalleNVR_IMPUESTO1_PORC: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO1_PORC'
      ReadOnly = True
    end
    object qyDetalleNVR_IMPUESTO2_PORC: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO2_PORC'
      ReadOnly = True
    end
    object qyDetalleNVR_IVA_DP: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IVA_DP'
      ReadOnly = True
    end
    object qyDetalleNVR_IEPS_DP: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IEPS_DP'
      ReadOnly = True
    end
    object qyDetalleNVR_IMPUESTO1_DP: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO1_DP'
      ReadOnly = True
    end
    object qyDetalleNVR_IMPUESTO2_DP: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO2_DP'
      ReadOnly = True
    end
    object qyDetalleNVR_SUBTOTAL_DP: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_SUBTOTAL_DP'
      ReadOnly = True
    end
    object qyDetalleNVR_IVA_DG: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IVA_DG'
      ReadOnly = True
    end
    object qyDetalleNVR_IEPS_DG: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IEPS_DG'
      ReadOnly = True
    end
    object qyDetalleNVR_IMPUESTO1_DG: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO1_DG'
      ReadOnly = True
    end
    object qyDetalleNVR_IMPUESTO2_DG: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_IMPUESTO2_DG'
      ReadOnly = True
    end
    object qyDetalleNVR_SUBTOTAL_DG: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_SUBTOTAL_DG'
      ReadOnly = True
    end
    object qyDetalleNVR_ESTATUS: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ESTATUS'
      ReadOnly = True
    end
    object qyDetalleNVR_TOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_TOTAL'
      ReadOnly = True
    end
    object qyDetalleNVR_DP: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DP'
      ReadOnly = True
    end
    object qyDetalleNVR_DG: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_DG'
      ReadOnly = True
    end
    object qyDetalleNVR_GT: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_GT'
      ReadOnly = True
    end
    object qyDetalleNVR_MATERIAL_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_MATERIAL_CLAVE'
      ReadOnly = True
    end
    object qyDetalleNVR_MATERIAL_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_MATERIAL_DESCRIPCION'
      ReadOnly = True
      Size = 100
    end
    object qyDetalleNVR_UNIDAD_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_UNIDAD_CLAVE'
      ReadOnly = True
      Size = 10
    end
    object qyDetalleNVR_UNIDAD_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_UNIDAD_DESCRIPCION'
      ReadOnly = True
      Size = 50
    end
    object qyDetalleNVR_VENDEDOR_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_VENDEDOR_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyDetalleNVR_CAJAS: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_CAJAS'
      ReadOnly = True
    end
    object qyDetalleNVdescripcion: TStringField
      FieldKind = fkCalculated
      FieldName = 'descripcion'
      Size = 100
      Calculated = True
    end
  end
  object spAUTO_FOLIO: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'AUTO_FOLIO'
    Left = 8
    Top = 273
    ParamData = <
      item
        DataType = ftInteger
        Name = 'R_FOLIO'
        ParamType = ptOutput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'P_MODULO'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'P_SUBCLASE'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftString
        Name = 'P_AFECTAR'
        ParamType = ptInput
        Value = 'No'
      end>
  end
  object spMODIFICAR_FOLIO: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'MODIFICAR_FOLIO'
    Left = 40
    Top = 273
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_MODULO'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'P_SUBCLASE'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'P_CONSECUTIVO'
        ParamType = ptInput
        Value = '0'
      end>
  end
  object spACTUALIZA_NOTA_DE_VENTA: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'ACTUALIZA_NOTA_DE_VENTA'
    Left = 72
    Top = 273
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_VENTA'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'P_ALMACEN'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'P_FOLIO'
        ParamType = ptInput
        Value = '0'
      end>
  end
  object qyNumeroCajas: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM NUMERO_CAJAS(:P_VENTA)')
    Left = 8
    Top = 242
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'P_VENTA'
        ParamType = ptUnknown
      end>
    object qyNumeroCajasR_NUM_CAJAS_COBRO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_NUM_CAJAS_COBRO'
      ReadOnly = True
    end
    object qyNumeroCajasR_NUM_CAJAS_NO_COBRO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_NUM_CAJAS_NO_COBRO'
      ReadOnly = True
    end
    object qyNumeroCajasR_TIPO_CAJA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_TIPO_CAJA'
      ReadOnly = True
    end
    object qyNumeroCajasR_MATERIAL_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_MATERIAL_CLAVE'
      ReadOnly = True
    end
    object qyNumeroCajasR_MATERIAL_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_MATERIAL_DESCRIPCION'
      ReadOnly = True
      Size = 100
    end
    object qyNumeroCajasR_PRECIO: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_PRECIO'
      ReadOnly = True
    end
    object qyNumeroCajasR_TOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_TOTAL'
      ReadOnly = True
    end
    object qyNumeroCajasR_ULTIMO: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ULTIMO'
      ReadOnly = True
      Size = 2
    end
  end
  object dsNumeroCajas: TDataSource
    DataSet = qyNumeroCajas
    Left = 40
    Top = 242
  end
  object qyVerificarFolio: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM VERIFICAR_FOLIO_NV(:P_ID, :P_FOLIO, :P_ALMACEN)')
    Left = 104
    Top = 273
    ParamData = <
      item
        DataType = ftString
        Name = 'P_ID'
        ParamType = ptUnknown
        Value = '0'
      end
      item
        DataType = ftString
        Name = 'P_FOLIO'
        ParamType = ptUnknown
        Value = '0'
      end
      item
        DataType = ftString
        Name = 'P_ALMACEN'
        ParamType = ptUnknown
        Value = '0'
      end>
    object qyVerificarFolioR_ACEPTADO: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_ACEPTADO'
      ReadOnly = True
      Size = 2
    end
  end
end
