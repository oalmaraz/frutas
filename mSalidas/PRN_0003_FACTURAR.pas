unit PRN_0003_FACTURAR;

interface
                      
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  padre,    Db, IBCustomDataSet, IBQuery,
  jpeg,  ExtCtrls,  StdCtrls, ComCtrls, Buttons, Grids, DBGrids;

type
  TfrmPRN_0003 = class(TfrmPadre)
...
constructor TfrmPRN_0003.CrearReImpresion(AOwner : TComponent; p_almacen : integer; x1, y1, y2 : integer);
begin
   Create(AOwner);
   Left   := x1;
   Top    := y1;
...
procedure TfrmPRN_0003.crearImpresion;
const
   PARTIDAS = 13;
var
   i : integer;
...
begin
   ...
   union     := qyEncabezado.FieldByName('R_CLIENTE_CLAVE').AsString+' '+qyEncabezado.FieldByName('R_CLIENTE_NOMBRE').AsString;
...
//   tpImpresion.WriteToPort(pedido);
   ...
   begin
      ...
      cantidad  := FormatFloat('###,###,##0.00',qyDetalle.FieldByName('R_KILOS').AsFloat);
...
constructor TfrmPRN_0003.Crear(AOwner : TComponent; p_venta : integer);
begin
   Create(AOwner);
...
procedure TfrmPRN_0003.dsBusquedaVentaDataChange(Sender: TObject;
  Field: TField);
begin
  inherited;
...
procedure TfrmPRN_0003.dsEncabezadoDataChange(Sender: TObject;
  Field: TField);
begin
  inherited;
  qyDetalle.Close;
...
procedure TfrmPRN_0003.ibBusquedaClick(Sender: TObject);
begin
  inherited;
  qyBusquedaVenta.Close;
...
procedure TfrmPRN_0003.ibImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmPRN_0003.dgFacturadosDblClick(Sender: TObject);
begin
  inherited;
